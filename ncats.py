...
# Train the network
nn.train(train_data, train_labels, epochs=151, batch_size=10)
...
# Guardar imágenes 
import os
os.makedirs('dataset/predict/cat', exist_ok=True)
os.makedirs('dataset/predict/nocat', exist_ok=True)

test_images = test_data.reshape(50, 64, 64, 3 )
 # (50, 64, 64, 3)

for i in range(50):
    if predictions[i][0] > 0.5:
        plt.imsave(f'dataset/predict/cat/img_{i+1}.png', test_images[i])
        print(f" Imagen {i+1} guardada en cat/")
    else:
        plt.imsave(f'dataset/predict/nocat/img_{i+1}.png', test_images[i])
        print(f" Imagen {i+1} guardada en nocat/")
